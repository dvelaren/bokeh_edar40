...
from bokeh.layouts import column, grid, layout, row, widgetbox
from bokeh.models import (Button, ColumnDataSource, DataTable, DatePicker,
                          DateRangeSlider, Div, Label, LabelSet, Span,
                          TableColumn)
...
def create_treemap(df):
    ...
    def create_first_level_treemap_data(df):
        ...
        source = create_treemap_data(
            indicator_values, sum_total_values, color_values)
        return source
    ...
    def create_low_level_treemap_data(df, indicator, color_value):
        ...
        if sum_value > 0:
            ...
            source = create_treemap_data(list(indicator_df['cluster']), list(
                indicator_df['valor']), [color_value]*len(indicator_df))
            return source
    ...
    def create_treemap_rects(total_source, x, y, width, height, rects, treemap_figure):
        ...
        values = normalize_sizes(
            total_source.data['Valor_Total'], width, height)
        ...
        treemap_figure.quad(top=YdY, bottom=Y, left=X, right=XdX,
                            color=total_source.data['Colores'], line_color='black')
    ...
    treemap_figure = figure(plot_height=400, sizing_mode='stretch_width',
    ...
    treemap_info = create_treemap_rects(
        first_level_source, 0., 0., 1, 1, [], treemap_figure)
    ...
    for indicador in indicadores['Indicador']:
        low_level_source = create_low_level_treemap_data(
            df, indicador, colores[i])
        if low_level_source != None:
            ...
            treemap_info = create_treemap_rects(
                low_level_source, rects[i]['x'], rects[i]['y'], rects[i]['dx'], rects[i]['dy'], rects, treemap_info['treemap'])
            new_rects = treemap_info['rects']
            new_rects_text = create_low_level_rect_text_data(
                new_rects, low_level_source, new_rects_text['x'], new_rects_text['y'], new_rects_text['text'])
            treemap_figure = treemap_info['treemap']
            i = i + 1

    # Creamos el texto para cada rectángulo y luego lo mostramos, primero nivel superior y después inferiores
    treemap_figure.text(rect_text['x'], rect_text['y'],
                        text=rect_text['text'], text_font_size={'value': '11pt'})
    treemap_figure.text(new_rects_text['x'], new_rects_text['y'],
                        text=new_rects_text['text'], text_font_size={'value': '11pt'})
...
def create_normalize_plot(df):
    ...
    source_cluster = [create_data_source_from_dataframe(
        df, 'cluster', f'cluster_{i}') for i in range(NUM_CLUSTERS)]
    ...
    hline = Span(location=1, dimension='width', line_color='red',
                 line_alpha=0.6, line_dash='dotted', line_width=2)

    normalize_plot.add_layout(hline)

    # Etiqueta linea horizontal
    hlabel = Label(x=500, y=1, x_units='screen', text='Max',
                   text_color='red', text_alpha=0.6, text_font_size='14px')
    normalize_plot.add_layout(hlabel)

    for i in range(NUM_CLUSTERS):
        normalize_plot.line(x='Indicador', y='valor', source=source_cluster[i], line_dash='dashed',
                            line_width=2, line_color=bokeh_utils.LINE_COLORS_PALETTE[i], legend_label=f'Cluster {i}')
        normalize_plot.circle(x='Indicador', y='valor', source=source_cluster[i], size=8,
                              line_color=bokeh_utils.LINE_COLORS_PALETTE[i], fill_color='white', legend_label=f'Cluster {i}')
...
def create_radar_plot(df, tipo_var):
    ...
    for i in range(GRID_STEPS):
        ...
        if i == 0:
            source = ColumnDataSource(
                {'x': x[i] + [CENTER], 'y': y[i] + [(GRID_STEPS-i)*0.5/GRID_STEPS+CENTER], 'text': text + ['']})
            # source_test = ColumnDataSource({'x':x_var_label + [CENTER],'y':y[i]+ [(GRID_STEPS-i)*0.5/GRID_STEPS+CENTER],'text':text+ ['']})
            var_labels = LabelSet(x="x", y="y", text="text", source=source,
                                  text_color=bokeh_utils.LABEL_FONT_COLOR, text_font_size='15px')  # Position variable labels
            nor_rad_pl.add_layout(var_labels)  # Add variable labels
            color = 'red'
            line_dash = 'dashed'
        else:
            # radius*i/GRID_STEPS+CENTER
            source = ColumnDataSource(
                {'x': x[i] + [CENTER], 'y': y[i] + [(GRID_STEPS-i)*0.5/GRID_STEPS+CENTER]})
            color = 'gainsboro'
            line_dash = 'solid'
        nor_rad_pl.line(x="x", y="y", source=source, line_color=color,
                        line_dash=line_dash)  # Create poligons
        y_ticks.append(y[i][0])  # Store y-vertices grid ticks positions

    for i in range(NUM_VARS):
        # Create grid lines from center to vertix
        nor_rad_pl.line(x=[CENTER, x[0][i]], y=[
                        CENTER, y[0][i]], line_color='gainsboro')

    y_ticks.reverse()
    text_labels_ticks = np.around(list(np.linspace(
        0, 1, GRID_STEPS, endpoint=False)), decimals=2)  # Create text for grid ticks
    source_labels_ticks = ColumnDataSource(
        {'x': x_ticks, 'y': y_ticks[:-1], 'text': text_labels_ticks[1:]})
    tick_labels = LabelSet(x="x", y="y", text="text", source=source_labels_ticks,
                           text_color=bokeh_utils.LABEL_FONT_COLOR, text_font_size='14px')  # Position grid tick labels
    ...
    for i, cluster in enumerate(df.cluster.unique()):
        ...
        xt, yt = radar_patch(
            (clist[i].valor-DATA_MIN)/(DATA_MAX-DATA_MIN) * 0.5, theta, CENTER)
        clist[i] = clist[i].assign(
            **{'xt': xt, 'yt': yt, 'valor_map': (clist[i].valor-DATA_MIN)/(DATA_MAX-DATA_MIN)})
        nor_rad_pl.patch(x='xt', y='yt', fill_alpha=0.15,
                         fill_color=colors[i], line_color=colors[i], legend_label=f'Cluster {i}', source=ColumnDataSource(clist[i]))
        nor_rad_pl.circle(x='xt', y='yt', size=15, fill_color=None,
                          line_color=None, source=ColumnDataSource(clist[i]), name='radar_plt')
...
def create_not_normalize_plot(df, tipo_var):
    ...
    data_table = DataTable(source=source, columns=columns, selectable=False,
                           sizing_mode='stretch_width', max_width=580, height=350)
...
def create_weight_plot(df):
    ...
    weight_plot = figure(max_width=650, height=400, toolbar_location=None, sizing_mode='stretch_width', y_range=FactorRange(
        factors=source.data['Attribute']), x_range=(0, 1), tooltips=TOOLTIPS, output_backend="webgl")
    weight_plot.hbar(y='Attribute', left='Weight', right=0, source=source, height=0.6,
                     fill_color=bokeh_utils.BAR_COLORS_PALETTE[0], line_color=bokeh_utils.BAR_COLORS_PALETTE[0])
...
def create_title(text):
    ...
    title = Div(text=text, style={'font-weight': 'bold', 'font-size': '16px', 'color': bokeh_utils.TITLE_FONT_COLOR,
                                  'margin-bottom': '2px', 'font-family': 'inherit'}, width=470, height=16)
    return title
...
def modify_first_descriptive(doc):
    ...
    normalize_df['Indicador'] = normalize_df['Indicador'].replace(
        regex=[r'\(', r'\)', 'average'], value='')
    not_normalize_df['Indicador'] = not_normalize_df['Indicador'].replace(
        regex=[r'\(', r'\)', 'average'], value='')
    ...
    profile_title = create_title(
        'Perfil de calidad del agua (normalizado) - Diagrama de línea')
    profile_title_rad = create_title(
        'Perfil de calidad del agua (normalizado) - Diagrama de araña')

    # Tabla sin normalizar
    not_normalize_table_title = create_title(
        'Indicadores influyentes sin normalizar')
    not_normalize_table = create_not_normalize_plot(not_normalize_df, tipo_var)
    not_normalize_widget_box = widgetbox(
        [not_normalize_table_title, not_normalize_table], max_width=600, height=400, sizing_mode='stretch_both', spacing=3)
    ...
    l = grid([
        [column([profile_title, normalize_plot], sizing_mode='stretch_width'), column(
            [profile_title_rad, nor_rad_pl], sizing_mode='stretch_width')],
