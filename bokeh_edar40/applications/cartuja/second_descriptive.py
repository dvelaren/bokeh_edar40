...
from bokeh.models import (BasicTicker, ColorBar, ColumnDataSource, Div,
                          GraphRenderer, HoverTool, Label, Legend, LinearAxis,
                          LinearColorMapper, MonthsTicker, MultiLine,
                          PrintfTickFormatter, Range1d, Rect, Span,
                          StaticLayoutProvider)
from bokeh.models.formatters import DatetimeTickFormatter
from bokeh.models.tickers import FixedTicker
from bokeh.models.widgets import Button, CheckboxButtonGroup, DataTable, Select
from bokeh.plotting import figure
from bokeh.transform import dodge, transform
from bokeh_edar40.visualizations.decision_tree import Node, Tree
from bokeh_edar40.visualizations.simul_optim_widgets import (SimulOptimWidget,
                                                             Spinner,
                                                             create_div_title)
from pandas.io.json import json_normalize
from utils.generate_model_vars import (load_obj, load_or_create_model_vars,
                                       save_obj)
    ...
    for i in range(num_vals):
        if start_x == 0:
            x = start_x + i * (bar_width + bar_width/2) - \
                ((bar_width*num_vals/2) + bar_width/2)
        else:
            x = start_x + i * (bar_width + bar_width/2) - \
                ((2*bar_width*num_vals/2) - bar_width/2)
            if i >= num_vals/2:
                x = x + (bar_width/2)
        x_pos.append(x)
...
def create_corrects_plot(df, target):
    ...
    Parameters:
		df (DataFrame): Dataframe con los datos de la matriz de confusión.
		target (str): Titulo.
    ...
    corrects_plot = figure(x_range=xlabels, plot_height=400, toolbar_location=None,
                           sizing_mode='stretch_width', output_backend="webgl")

    bar_width = 0.1
    xloc = calc_xoffset_corrects_plot(
        num_vals=len(xlabels), bar_width=bar_width)
...
def create_attribute_weight_plot(df, target):
    ...
    weight_plot = figure(plot_height=400, toolbar_location=None, sizing_mode='stretch_width',
                         x_range=df['Attribute'].values, output_backend="webgl")

    weight_plot.vbar(x='Attribute', top='Weight', source=source,
                     width=0.9, line_color='white', fill_color='colors')
...
def create_confusion_matrix(df):
    ...
    mapper = LinearColorMapper(
        palette=colors, low=data_dict.value.min(), high=data_dict.value.max())
...
def create_model_menu(model_variables=[]):
    ...
    select = Select(value=selected_value, options=option_values,
                    height=35, min_width=180, max_width=180)
    button = Button(label='Modelizar', button_type='primary',
                    height=35, min_width=390, max_width=390)
...
def create_decision_tree_graph_renderer(plot, tree):
    ...
    graph.node_renderer.glyph = Rect(
        height=0.15, width=0.2, fill_color='color')
    graph.edge_renderer.glyph = MultiLine(
        line_color='#b5b8bc', line_alpha=0.8, line_width=5)
...
def append_labels_to_decision_tree(plot, graph, tree):
    ...
    plot.text(node_text_x, node_text_y, text=node_text,
              text_font_size={'value': '10pt'}, text_align='center')

    middle_x, middle_y, middle_text = tree.get_line_text_positions()
    plot.text(middle_x, middle_y, text=middle_text, text_font_size={
              'value': '10pt'}, text_align='center')
    return plot
...
def create_decision_tree_plot():
    ...
    Returns:
		Figure: Gráfica del árbol de decisión.
    """
    plot = figure(x_range=(-1.1, 1.1), y_range=(0, 1.1), toolbar_location=None,
                  plot_height=800, sizing_mode='stretch_width', output_backend="webgl", tools="")
...
def create_outlier_plot(df, prediction_df, tipo_var):
    ...
    if tipo_var == 'RENDIMIENTOS':
        tools = [
            ('Fecha', '@Fecha{%F}'),
            ('Outlier', '@outlier'),
            ('efluente_rend_elim_DBO5',
             "Val: @efluente_rend_elim_DBO5, Prom: @{mode(efluente_rend_elim_DBO5)}, Anom: @efluente_rend_elim_DBO5_anomalia"),
            ('efluente_rend_elim_DQOt',
             'Val: @efluente_rend_elim_DQOt, Prom: @{mode(efluente_rend_elim_DQOt)}, Anom: @efluente_rend_elim_DQOt_anomalia'),
            ('efluente_rend_elim_NTK',
             'Val: @efluente_rend_elim_NTK, Prom: @{mode(efluente_rend_elim_NTK)}, Anom: @efluente_rend_elim_NTK_anomalia'),
            ('efluente_rend_elim_Pt',
             'Val: @efluente_rend_elim_Pt, Prom: @{mode(efluente_rend_elim_Pt)}, Anom: @efluente_rend_elim_Pt_anomalia'),
            ('efluente_rend_elim_SST',
             'Val: @efluente_rend_elim_SST, Prom: @{mode(efluente_rend_elim_SST)}, Anom: @efluente_rend_elim_SST_anomalia')
        ]
        ...
        tools = [
            ...
            ('Outlier', '@outlier'),
            ('efluente_DBO5t_conc',
             "Val: @efluente_DBO5t_conc, Prom: @{mode(efluente_DBO5t_conc)}, Anom: @efluente_DBO5t_conc_anomalia"),
            ('efluente_DQOt_conc',
             'Val: @efluente_DQOt_conc, Prom: @{mode(efluente_DQOt_conc)}, Anom: @efluente_DQOt_conc_anomalia'),
            ('efluente_Ntk_conc',
             'Val: @efluente_Ntk_conc, Prom: @{mode(efluente_Ntk_conc)}, Anom: @efluente_Ntk_conc_anomalia'),
            ('efluente_Pt_conc',
             'Val: @efluente_Pt_conc, Prom: @{mode(efluente_Pt_conc)}, Anom: @efluente_Pt_conc_anomalia'),
            ('efluente_MES_conc',
             'Val: @efluente_MES_conc, Prom: @{mode(efluente_MES_conc)}, Anom: @efluente_MES_conc_anomalia')
        ]
    hover_tool = HoverTool(
        tooltips=tools,
        formatters={
    ...
    outlier_plot = figure(plot_height=400, toolbar_location=None,
                          sizing_mode='stretch_width', x_axis_type='datetime', output_backend="webgl")

    df['Fecha'] = pd.to_datetime(df['Fecha'])
    df['outlier'] = pd.to_numeric(pd.Series(df['outlier'].values))

    prediction_df['añomes'] = pd.to_datetime(
        prediction_df['añomes'], format='%m/%d/%y %I:%M %p').dt.normalize()
    ...
    size = 5
    alpha = 0.4
    outlier_plot.circle(x='Fecha', y='outlier', source=source_cluster_0,
                        color=bokeh_utils.LINE_COLORS_PALETTE[0], alpha=alpha, size=size, legend_label='Cluster 0')
    outlier_plot.circle(x='Fecha', y='outlier', source=source_cluster_1,
                        color=bokeh_utils.LINE_COLORS_PALETTE[1], alpha=alpha, size=size, legend_label='Cluster 1')
    outlier_plot.circle(x='Fecha', y='outlier', source=source_cluster_2,
                        color=bokeh_utils.LINE_COLORS_PALETTE[2], alpha=alpha, size=size, legend_label='Cluster 2')
    outlier_plot.circle(x='Fecha', y='outlier', source=source_cluster_3,
                        color=bokeh_utils.LINE_COLORS_PALETTE[3], alpha=alpha, size=size, legend_label='Cluster 3')
    ...
    outlier_plot.x_range = Range1d(prediction_df['añomes'].min(
    )-pd.DateOffset(months=1), prediction_df['añomes'].max()+pd.DateOffset(months=1))
...
def create_prediction_plot(df):
    ...
    prediction_plot = figure(plot_height=400, toolbar_location=None,
                             sizing_mode='stretch_width', x_axis_type='datetime', output_backend="webgl")
    ...
    prediction_plot.line(x='añomes', y='Prediction', source=source_cluster_0, line_width=2,
                         line_color=bokeh_utils.LINE_COLORS_PALETTE[0], legend_label='Cluster 0')
    prediction_plot.line(x='añomes', y='Prediction', source=source_cluster_1, line_width=2,
                         line_color=bokeh_utils.LINE_COLORS_PALETTE[1], legend_label='Cluster 1')
    prediction_plot.line(x='añomes', y='Prediction', source=source_cluster_2, line_width=2,
                         line_color=bokeh_utils.LINE_COLORS_PALETTE[2], legend_label='Cluster 2')
    prediction_plot.line(x='añomes', y='Prediction', source=source_cluster_3, line_width=2,
                         line_color=bokeh_utils.LINE_COLORS_PALETTE[3], legend_label='Cluster 3')
    ...
    prediction_plot.xaxis[0].formatter = DatetimeTickFormatter(months=[
                                                               '%b %Y'])
    ...
    vlabel = Label(x=prediction_date, y=25, text='→Predicción',
                   text_color='gray', text_alpha=0.6, text_font_size='14px')
...
def append_count(df):
    ...
    for row in range(len(df.index)):
        test_df = df.loc[row, df.columns.str.contains(
            'range|cluster')].sort_values(ascending=False)
        sorted_text = []
...
def create_daily_pred_plot(df_original, target='Calidad_Agua'):
    ...
    df = df_original
    df = df.rename(
        columns={target: 'real', f'prediction({target})': 'prediction'})
    ...
    daily_pred_plot.extra_y_ranges = {
        'y_error': Range1d(start=0, end=len(bins))}
    daily_pred_plot.add_layout(LinearAxis(
        y_range_name='y_error', axis_label='Error', ticker=list(range(len(bins)))), 'right')

    daily_pred_plot.line(x='Fecha', y='real', source=source, line_width=2,
                         line_color='#392FCC', line_alpha=0.8, legend_label='Real')
    daily_pred_plot.line(x='Fecha', y='prediction', source=source, line_width=2,
                         line_color='#CA574D', line_alpha=0.8, line_dash='dashed', legend_label='Predicción')
    daily_pred_plot.line(x='Fecha', y='error', source=source, line_width=2,
                         line_color='green', line_alpha=0.4, legend_label='Error', y_range_name='y_error')
    ...
    if target == 'Calidad_Agua':
        ...
        daily_pred_plot.yaxis[0].formatter = PrintfTickFormatter(
            format="Cluster %u")
    else:
        daily_pred_plot.yaxis[0].ticker = list(range(1, 1+len(bins)))
        daily_pred_plot.y_range = Range1d(
            0, len(bins)+1)  # Manipulates y_range
        daily_pred_plot.yaxis[0].formatter = PrintfTickFormatter(
            format="Range %u")
...
def create_df_sliders(weight_df, pred_df):
    ...
    Parameters:
		weight_df (DataFrame): Dataframe con las variables influyentes.
		pred_df (DataFrame): Dataframe con las predicciones diarias.

    Returns:
		slider_df (DataFrame): Dataframe con los valores minimo, medio y máximo para la creación de sliders.
    """
...
def create_div_text(text):
    ...
    Returns:
		div_text (Div): Div con el texto formateado.
    """
    div_text = Div(
        text=text,
        style={
...
def create_ranges_description(possible_targets, target='Calidad_Agua'):
    ...
    Parameters:
		possible_targets (str): Lista con los posibles valores para los rangos.

    Returns:
		col (column): Columna con todos los rangos posibles.
    """
...
def modify_second_descriptive(doc):
    ...
    total_model_dict = load_or_create_model_vars(model_vars_file='resources/total_model_dict.pkl',
                                                 ...
                                                 sheets=['ID_INFLUENTE',
                                                         ...
                                                         'METEO'],
                                                 cols=[
                                                     'OUT', 'IN', 'MANIPULABLES', 'PROCESOS_IN'],
                                                 force_create=False)
    ...
    recreate_button = Button(
        label='Recrear', button_type='success', height=35, max_width=200, min_width=200)
    model_select_wb = widgetbox(
        [
            row([model_title, create_model_spinner.spinner],
                sizing_mode='stretch_width', max_width=400),
    ...
    created_models_checkbox = CheckboxButtonGroup(
        labels=list(models.keys()), height=35)
    created_models_checkbox.active = [0]
    delete_model_button = Button(
        label='Eliminar', button_type='danger', height=35, max_width=200)
    created_models_wb = widgetbox(
        [created_models_title, created_models_checkbox], max_width=900, sizing_mode='stretch_width')
    ...
    def prediction_callback(model='Calidad_Agua'):
        ...
        if model_objective not in models:
            ...
            # Obtener datos
            df_prediction = [json_normalize(data)
                             for data in json_prediction_document]
            ...
            weight_plot = create_attribute_weight_plot(
                weight_df, model_objective)
            ...
            new_plots = layout([
                ...
                [row([simul_or_optim_wb.wb, ranges_description],
                     min_width=1400, sizing_mode='stretch_width')],
                [daily_pred_plot],
                [column([confusion_title, confusion_matrix],
                        sizing_mode='stretch_width'), weight_plot, corrects_plot],
                [decision_tree_title],
                [decision_tree_plot]
    ...
    def remove_model_handler(new):
        selected_labels = [created_models_checkbox.labels[elements]
    ...
    l = layout([
        ...
        [outlier_plot],
        [simulation_title],
        [model_select_wb, column(
            created_models_wb, delete_model_button, sizing_mode='stretch_width')],
        [model_plots]
