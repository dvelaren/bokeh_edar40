...
def flatten(A):
    ...
    Parameters:
        A (any): The list to be flattened.
...
def create_vars_mask_df(column_names, vars):
    """Function to create vars mask df.

    Parameters:
        column_names (str): List of column names for header.
        vars (dict): Dictionary with variables to create dataframe.

    Returns:
        DataFrame: Dataframe with column names and vars.
    """
...
def create_meteo_df(units,
    ...
    """ Creates meteo period 2 Dataframe from static folders.

    Parameters:
        units (dict): Units dictionary for each variable.
        year_folders (str): List with year folders to read.
        year_months (str): List with month folders to read.
        column_names (dict): Column names mapping dictionary.
        in_data_file_dir (Path): Path to meteo files directory.
        data_file_names (dict): Dictionary with variables and corresponding file name.
        meteo_period2_file_name (Path): Path to meteo period 2 output file.
        meteo_period2_sheet_name (str): Period 2 desired sheet name.

    Returns:
        DataFrame: Meteo Period 2 Dataframe.
    """
    ...
    for year in year_folders:
        for month in year_months[year]:
            ...
            for col in column_names:
                ...
                df_list.append(df)
            df_tot = reduce(lambda df1, df2: pd.merge(
                df1, df2, on='Fecha', how='inner'), df_list)
...
def create_meteo_live_df(
    ...
    """ Creates meteo period 2 Dataframe with new live data.

    Parameters:
        meteo_period2_file_name (Path): Path to meteo period 2 input file.
        meteo_period2_sheet_name (str): Period 2 sheet name.
        meteo_live_file_name (Path): Path to meteo period 2 output file.
        live_file_column_names (str): Column names mapping dictionary.
        live_file_sheet_name (str): Period 2 desired sheet name.

    Returns:
        DataFrame: Meteo Period 2 live Dataframe.
    """
    ...
    # Obtain last stored timestamp
    last_timestamp = df_period_2.index[-1].to_pydatetime().date()
    today = datetime.now().date()  # Obtain today timestamp
    # Compute remaining days to be compleated with live file
    remaining_timestamps = pd.date_range(
        start=last_timestamp+timedelta(days=1), end=today-timedelta(days=1), freq='d')
    ...
    # Convert precipitation [cm -> mm]
    df_meteo_live_units[live_file_column_names['P24']
                        ] = df_meteo_live[live_file_column_names['P24']] * 10
    # Convert temperature [°C -> (1/10)°C]
    df_meteo_live_units[live_file_column_names['TMED']
                        ] = df_meteo_live[live_file_column_names['TMED']] * 10
    # Convert pressure [kPa -> hPa]
    df_meteo_live_units[live_file_column_names['PRES']
                        ] = df_meteo_live[live_file_column_names['PRES']] * 100
    df_meteo_live_units = df_meteo_live_units.astype('int32')

    # Build dataframe with remaining timestamps
    df_new_data = df_meteo_live_units.loc[df_meteo_live_units.index.intersection(
        remaining_timestamps)]

    # Add new data to PERIOD_2
    df_period_2 = df_period_2.append(
        df_new_data, sort=True, verify_integrity=True)
    df_period_2.index.name = 'Fecha'
    df_period_2.to_excel(meteo_period2_file_name,
                         sheet_name=meteo_period2_sheet_name)
...
def Create_Partial_DF(variables_file_name,
                      ...
                      blnRename_columns,
                      blnConsider_UNITS):
    """Auxiliary function for creating partial DF according to variables to be read from COMPLETE DF.

    Parameters:
        variables_file_name (Path): Path to variables file name.
        variables_sheet_name (str): Variables sheet name.
        vars_ORIGEN_col_name (str): Origin variables column name.
        vars_DESTINO_col_name (str): Destiny variables column name.
        df_COMPLETE (DataFrame): DataFrame of all data.
        original_vars_list_COMPLETE (str): List of variables column names from original data.
        blnRename_columns (bool): Rename columns with new names boolean parameter.
        blnConsider_UNITS (bool): Consider units row boolean parameter.

    Returns:
        df_PARTIAL (DataFrame): Processed partial DataFrame.
        columns_with_all_NaNs_list (str): List of columns which had NaNs in all of its rows.
        columns_not_found_in_original_list (str): List of columns not found in original list.
    """
    # Local variables
    ...
    df_vars = pd.read_excel(io=variables_file_name,
                            sheet_name=variables_sheet_name)
    ...
    if (blnRename_columns == True):
        ...
        for i in range(0, len(df_vars_ORIGEN_col_names)):
            df_PARTIAL.rename(
                columns={df_vars_ORIGEN_col_names[i]: df_vars_DESTINO_col_names[i]}, inplace=True)
