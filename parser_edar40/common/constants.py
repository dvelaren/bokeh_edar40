...
VARS_NORMA_ABSOLUTAS = {'efluente_MES_conc': 35, 'efluente_DBO5t_conc': 25,
                        'efluente_DQOt_conc': 125, 'efluente_Pt_conc': 1, 'efluente_Ntk_conc': 10}
VARS_NORMA_RENDIMIENTOS = {'efluente_rend_elim_SST': 0.7, 'efluente_rend_elim_DBO5': 0.7,
                           'efluente_rend_elim_DQOt': 0.75, 'efluente_rend_elim_Pt': 0.8, 'efluente_rend_elim_NTK': 0.9}
...
YEAR_FOLDERS = [f.name for f in os.scandir(
    IN_METEO_DATA_FILE_DIR) if f.is_dir()]
...
YEAR_MONTHS = {year: (MONTH_FOLDER_NAMES[4:] if year == '2018' else MONTH_FOLDER_NAMES[0:len(
    [f.name for f in os.scandir(IN_METEO_DATA_FILE_DIR / year) if f.is_dir()])]) for year in YEAR_FOLDERS}
...
COLUMN_NAMES = {'P24': ['P24'], 'TMED': ['TMED'],
...
UNITS = {'Fecha': 'NaT', 'P24': 'mm', 'TMED': '(1/10)C', 'PRES00': '(1/10)hPa',
         'PRES07': '(1/10)hPa', 'PRES13': '(1/10)hPa', 'PRES18': '(1/10)hPa'}
...
VARIABLES_TO_READ_FILE_NAME = Path(
    './data/EDAR4.0_EDAR_Cartuja_VARIABLES_V5.0.xlsx')
